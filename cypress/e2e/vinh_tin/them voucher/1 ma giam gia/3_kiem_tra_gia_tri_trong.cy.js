describe('Kiểm tra nhập dữ liệu trống', () => {
	beforeEach(() => {
		// mở web
	...
	it('Nhập giá trị không trống', () => {
		cy.get('input[ng-model="voucher.Code"]').type('HFGGF');
			...
			if (apiCalled) {
				cy.wait('@postVoucher').then(interception => {
					...
					if (response.statusCode == 200) {
						...
						throw new Error(`Lỗi: không thể thêm voucher mặc dù nhập giá trị đúng`);
					}
				});
			} else {
				cy.log('Không có request API nào được gọi, có thể form đã validate frontend.');
